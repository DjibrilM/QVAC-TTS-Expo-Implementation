...
interface WhisperModelLoaderProps {
  onDownload: () => void;
  isDownloading: boolean;
  progress: number; // 0 to 1
}

export function WhisperModelLoader({
  onDownload,
  isDownloading,
  progress,
}: WhisperModelLoaderProps) {
  const percentage = Math.round(progress * 100);
  const totalSize = 74.1; // MB
      ...
      <View className="w-full max-w-sm p-6 bg-card rounded-2xl border border-border shadow-lg">
        ...
        <View className="items-center mb-6 gap-3">
          ...
          <Text className="text-sm text-muted-foreground text-center leading-relaxed">
            Download the offline speech recognition model to transcribe voice recordings
